...
import { ZemenTaskPill } from '@zemen/react';
...
const CONTROLS: ControlDef[] = [
  ...
  { type: 'boolean', prop: 'showTime', label: 'Show Time', defaultValue: false },
...
const generateCode: CodeGenerator = (state: ControlState): string => {
  ...
  const lines: string[] = ['<ZemenTaskPill'];
  lines.push(`  task={{ id: '${task.id}', title: '${task.title}', ... }}`);
  if (state.showTime && task.time) {
    lines.push('  showTime');
  }
  if (state.draggable) {
...
export function ZemenTaskPillPlayground(): React.JSX.Element {
  return (
    <ComponentPlayground
